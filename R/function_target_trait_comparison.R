...
target_trait_comparison <- function(ranks, target.trait.file, target.trait.name, metadata = NULL) {
  ...
  if (!is.null(ranks[["SPATS_variables"]])) {
    if (!all(ranks[["SPATS_variables"]] %in% names(target.trait.file))) {
      stop(paste0(
        "The MulstispeQ ranks obtained by the function MSPQ_ranks() were adjusted by SpATS. The spatial variables ", paste0("'", ranks[["SPATS_variables"]], "'", collapse = ", "),
      ...
      stop(paste0(
        "The variable(s) ", paste0("'", c(ranks$Sources_of_variation[DAS_ind:length(ranks$Sources_of_variation)], target.trait.name)[pos], "'", collapse = ", "),
    ...
    for (i in 1:length(yield.list)) {
      yield.model[[i]] <- suppressWarnings(SpATS_mrbean(
        data = yield.list[[i]], response = target.trait.name, genotype = ranks$Genotype, col = ranks$SPATS_variables[2], row = ranks$SPATS_variables[1], segm = T, ncols = NULL, nrows = NULL, rep = "",
  ...
  for (i in 1:length(conam)) {
    ...
    if ("DAS" %in% ranks$Sources_of_variation) {
      conf_matrices[[i]] <- plotly::ggplotly(ggplot2::ggplot(
        aaa,
        ggplot2::aes_string(
          x = "factor(clus.y)", y = "factor(clus.m)", fill = "factor(col)",
          text = "name"
        )
      ) +
        ...
        ggplot2::labs(
          title = paste0(data_loop$DAS[1], " ", as.character(data_loop$time[1]), " ", unique(data_loop[, arr_SoV])),
          x = "Yield cluster", y = "MultispeQ cluster", fill = ""
        ), tooltip = "text")
      ...
      conf_matrices[[i]] <- plotly::ggplotly(ggplot2::ggplot(
        ...
        ggplot2::theme_classic() +
        ggplot2::labs(
          title = paste0(data_loop$date[1], " ", as.character(data_loop$time[1]), " ", unique(data_loop[, arr_SoV])),
          x = "Yield cluster", y = "MultispeQ cluster", fill = ""
        ), tooltip = "text")
