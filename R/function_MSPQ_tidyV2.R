...
MSPQ_tidy <- function(df, genotype, time.diff, data_name = NULL, plotIm = FALSE){
  ...
  namescol <- c("ID",                            "Series",                   "Repeat",               "Ambient.Humidity",
  ...
  if(summary_table$perc_removed_obs!=0){
    rm_table <- rm_data %>% dplyr::select_(.dots = c("date", SoV_names)) %>% table %>% as.data.frame
  ...
  if(plotIm){
    ...
    names(out) <- c("numeric_dataset", "non_numeric_dataset", "summary", "removed_observations", "removed_freq",
    ...
    names(out) <- c("numeric_dataset", "non_numeric_dataset", "summary", "removed_observations", "removed_freq", "Sources_of_Variation",
